...
class SeasonControllerTest {
    ...
    @Test
    void create_asTreasurer_returns403() throws Exception {
        ...
        verify(seasonService, never()).create(any(), anyLong());
    }

    @Test
    void create_asAdmin_returns201() throws Exception {
        when(seasonService.create(any(), anyLong())).thenReturn(buildSeasonResponse());
    ...
    void update_asAdmin_returns200() throws Exception {
        when(seasonService.update(eq(1L), any(), eq(1L))).thenReturn(buildSeasonResponse());
        ...
        verify(seasonService).update(eq(1L), any(), eq(1L));
    }

    @Test
    void update_asTreasurer_returns403() throws Exception {
        mockMvc.perform(put("/api/seasons/1")
                        .with(user(principalFor(User.Role.TREASURER)))
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(buildUpdateRequest())))
    ...
    private UpdateSeasonRequest buildUpdateRequest() {
        UpdateSeasonRequest r = new UpdateSeasonRequest();
        r.setName("2024/2025 Updated");
        r.setDateFrom(LocalDate.of(2024, 9, 1));
        r.setDateTo(LocalDate.of(2025, 6, 30));
        return r;
    }
